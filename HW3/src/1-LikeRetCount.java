...
public class LikeRetCount {
    ...
    public static class LRMapper
        ...
        public void map(Object key, Text value, Context context)
			...
			if(source.contains("Web App")){
				out_value.set(lCount + " " + rCount + " " + 1 + " " + 0 + " " + 0);
			}
			else if(source.contains("iPhone")){
				out_value.set(lCount + " " + rCount + " "  + 0 + " " + 1 + " " + 0);
			}
			else if(source.contains("Android")){
				out_value.set(lCount + " " + rCount + " "  + 0 + " " + 0 + " " + 1);
			}
			else {
				out_value.set(lCount + " " + rCount + " "  + 0 + " " + 0 + " " + 0);
			}
    ...
    public static class LRReducer
	    ...
	    int androidCount = 0;
            ...
            for (Text val : values) {
				...
				webAppCount += Integer.parseInt(splits[2]);
				iphoneCount += Integer.parseInt(splits[3]);
				androidCount += Integer.parseInt(splits[4]);
}
            // wrting into the context
            result.set(lCount + " " + rCount + " " + webAppCount+ " " + iphoneCount+ " " + androidCount);
